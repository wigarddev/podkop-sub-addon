...
Собирается в CI — `.github/workflows/build.yml`, официальный `openwrt/gh-action-sdk`.
Пакеты складываются в артефакты сборки, а по тегу `v*` прикрепляются к релизу.
Локально нужен OpenWrt SDK под Linux (WSL или Docker).
