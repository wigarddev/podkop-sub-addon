...
- Заголовок `x-hwid` для панелей, которые прячут список за лимитом устройств
- Серверы с транспортом, который podkop не соберёт, отсеиваются — мёртвых выходов в пуле не будет
...
> [!WARNING]
> Понимается только подписка-список ссылок — обычным текстом или в base64.
> Конфиги-объекты не разбираются: ни JSON для xray (панели отдают его клиентам
> вроде Happ), ни JSON для sing-box, ни YAML для Clash.
